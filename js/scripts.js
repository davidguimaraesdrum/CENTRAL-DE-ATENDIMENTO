...
function PLANTAO() {
  ...
  if(now.getDay() == 0 || holyday){ // Valida se é domingo ou se a variável holyday é true
    if((noon.includes(now.getHours()) || afternoon.includes(now.getHours()) || night.includes(now.getHours()))){ //Valida se a hora atual está nos horários definidos
      document.getElementById("plantao").disabled = false;	//Caso a validação anterior for verdadeira, o elemento que tem id plantao receberá false no campo disabled e o botão fica attivo
    }else{
      document.getElementById("plantao").disabled = true; //Caso a validação antetrior for falsa, o elemento recebe true, deixando o botão inativo
    }
  }
  ...
  if((now.getDay() > 0 && now.getDay() < 6) && !holyday){ //Valida se é entre segunda e sexta e não é feriado
    if(night.includes(now.getHours())){
      document.getElementById("plantao").disabled = false;	
    }else{
